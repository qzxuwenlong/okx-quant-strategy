...
class NewBacktester(Backtester):
    ...
    def update_positions_with_trailing(self, row):
        """更新持仓（带移动止损）"""
        for pos in self.open_positions:
            ...
            # 计算当前盈亏
            if pos['signal'] in ['buy', 'strong_buy']:
                pnl_pct = (current_price - entry_price) / entry_price
            else:  # short
                pnl_pct = (entry_price - current_price) / entry_price
            
            # 移动止损：盈利超过ATR×2时，止损移到成本价
            if atr > 0 and pnl_pct > (atr * 2 / entry_price):
                pos['stop_loss'] = entry_price  # 移到成本价
            
            # 更新未实现盈亏
            if pos['signal'] in ['buy', 'strong_buy']:
                pos['unrealized_pnl'] = (current_price - entry_price) * pos['size']
            else:  # short
                pos['unrealized_pnl'] = (entry_price - current_price) * pos['size']
    
    def run_backtest_new(self, symbol, proxy=None):
        """运行回测（新策略）"""
        print("=" * 60)
        print("回测: " + symbol + " (优化1: 移动止损)")
        print("=" * 60)
        ...
        for i in range(50, len(df)):
            ...
            # 更新持仓（带移动止损）
            self.update_positions_with_trailing(row)
...
if results:
    ...
    print("对比基准策略")
    print("=" * 60)
    print("基准: 收益1.40%, 胜率59.57%, 盈亏比1.71")
    print(f"优化1: 收益{results['total_return']:.2f}%, 胜率{results['win_rate']:.2f}%, 盈亏比{results['profit_factor']:.2f}")
    
    # 判断是否改进
    if results['total_return'] > 1.40:
        print("✅ 改进！收益提升")
    else:
        print("❌ 退步，收益下降")
